...
const addressPost = (req, res) => (
  req.scope.resolve(TYPES.Address).create(req.body)
    .then(result => (
...
const addressGet = (req, res) => (
  req.scope.resolve(TYPES.Address).fetch()
    .then(result => (
...
const addressGetById = (req, res) => (
  req.scope.resolve(TYPES.Address).fetchSingle(req.params.addressid)
    .then(result => (
      res.status(200).send({ data: result.data })
    ))
    .catch((err) => {
...
const addressPut = (req, res) => (
  req.scope.resolve(TYPES.Address).update(req.params.addressid, req.body)
    .then(result => (
...
const addressDelete = (req, res) => (
  req.scope.resolve(TYPES.Address).deleteSingle(req.params.addressid, req.body)
    .then(result => (
      res.status(200).send(result.data)
    ))
    .catch(err => (
