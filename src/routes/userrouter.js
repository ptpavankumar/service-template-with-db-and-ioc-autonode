...
const userPost = (req, res) => {
  return req.scope.resolve(TYPES.User).create(req.body)
      .then(result => (
...
const userGet = (req, res) => {
  try {
    req.scope.resolve(TYPES.User).fetch()
    .then(result => (
...
const userGetById = (req, res) => {
  req.scope.resolve(TYPES.User).fetchSingle(req.params.userid)
      .then(result => {
        console.log(result);
...
const userPut = (req, res) => (
  req.scope.resolve(TYPES.User).update(req.params.userid, req.body)
  .then(result => (
...
const userDelete = (req, res) => (
  req.scope.resolve(TYPES.User).deleteSingle(req.params.userid, req.body)
  .then(result => (
    res.status(200).send(result.data)
  ))
  .catch(err => (
